...
<head>
		...
		<meta name="theme-color" content="#BBD8B0"/>
		
		<script src="indexFolder/questScr.js"></script>
...
$sql = "SELECT id, form, Filling_Fee FROM test";
$temp = "CREATE TEMPORARY TABLE tempimmg LIKE test";




$query = "SELECT * FROM forms.test";

if ($stmt = $db->prepare($query)) {
    $stmt->execute();
    $stmt->bind_result($field1, $field2, $field3, $field4, $field5);
	echo "<div class='tg-wrap'><table id='tg-EhH7C' class='tg'>";
	echo "<table id='tg-EhH7C' class='tg'>\n";
	echo"<tr>";
    echo"<th class='tg-88fa'>FORM</th>";
    echo"<th class='tg-88fa'>FILLING FEE</th>";
    echo"<th class='tg-88fa'>BIOMETRICS FEE</th>";
    echo"<th class='tg-88fa'>TOTAL FEES</th>";
	echo"</tr>";
    while ($stmt->fetch()) {
		 echo "<tr>\n"
		  ."<td>{$field2}</td>\n"
		  ...
		  ."<td>\${$field5}</td>\n"
		  ."</tr>\n";
        //printf("%s, %s, %s, %s<br/>", $field2, $field3, $field4, $field5);
    }
	echo '</table></div>'; 
    $stmt->close();
}
